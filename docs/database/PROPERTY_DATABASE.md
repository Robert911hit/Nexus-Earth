# Property Database Specification

## Purpose

Defines production rules for storing and expanding Nexus Earth property data across Godot, FastAPI, PostgreSQL/Supabase, and the Next.js website.

## ID Format

Use `PRP-###` for authored content and `PRP-{region}-{shard}-{uuid}` for live player-scale records. IDs are immutable, lowercase slugs are display helpers only, and database primary keys must never depend on names.
