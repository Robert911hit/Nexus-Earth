# Item Database Specification

## Purpose

Defines production rules for storing and expanding Nexus Earth item data across Godot, FastAPI, PostgreSQL/Supabase, and the Next.js website.

## ID Format

Use `ITM-###` for authored content and `ITM-{region}-{shard}-{uuid}` for live player-scale records. IDs are immutable, lowercase slugs are display helpers only, and database primary keys must never depend on names.
