# Vehicle Database Specification

## Purpose

Defines production rules for storing and expanding Nexus Earth vehicle data across Godot, FastAPI, PostgreSQL/Supabase, and the Next.js website.

## ID Format

Use `VEH-###` for authored content and `VEH-{region}-{shard}-{uuid}` for live player-scale records. IDs are immutable, lowercase slugs are display helpers only, and database primary keys must never depend on names.
